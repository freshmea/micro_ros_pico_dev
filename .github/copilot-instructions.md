...
└── config/             # 설정 헤더 (lwipopts.h, rcl_check_macros.h)
```
...
#include "rcl_check_macros.h" // src/config/에 위치
```

### 6. 설정 헤더 파일
- **lwipopts.h**: lwIP (경량 TCP/IP 스택) 설정 - `src/config/`에 위치
- **rcl_check_macros.h**: micro-ROS 에러 체크 매크로 (RCCHECK, RCSOFTCHECK) - `src/config/`에 위치
...
├── src/
│   ├── main.c
│   ├── board/          (2 files - 하드웨어 추상화)
│   ├── drivers/        (4 files - 서보 드라이버)
│   ├── transport/      (4 files - WiFi/UART 전송)
│   ├── uros/           (2 files - micro-ROS 앱)
│   └── config/         (2 files - 설정 헤더)
├── example/            (예제 및 테스트 프로그램)
├── libmicroros/        (미리 컴파일된 라이브러리)
├── libfixmath/         (고정소수점 연산 라이브러리)
└── CMakeLists.txt

총 15개 소스 파일 (src/), 5개 디렉토리
```

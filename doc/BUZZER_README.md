...
- 최신 런타임(`uros_main`)에서는 **버저 제어를 periph_task에서 직접 수행**합니다.
- `/buzzer` 토픽 기반 제어는 `uros_app` 참고 구현에 포함되어 있습니다.
- ROS 토픽으로 버저를 제어하려면 `uros_app` 경로를 활성화하거나 `uros_main`에 구독자를 추가해야 합니다.

...
foreach(SEARCH_PASS RANGE 0 1)
    ...
    if(PICO_PLATFORM STREQUAL "rp2040")
        ...
        # For RP2350 platforms, use RP2040 port (compatible)
        # RP2350 specific ports may not be available yet
        if (PICO_PLATFORM STREQUAL "rp2350-riscv")
            # Try RISC-V specific port first, fallback to RP2040
            if (EXISTS "${FREERTOS_KERNEL_PATH}/${FREERTOS_KERNEL_RP2040_RELATIVE_PATH}/RP2350_RISC-V/CMakeLists.txt")
                set(FREERTOS_KERNEL_RP2040_RELATIVE_PATH "${FREERTOS_KERNEL_RP2040_RELATIVE_PATH}/RP2350_RISC-V")
            else()
                message(STATUS "RP2350 RISC-V port not found, using RP2040 port")
                set(FREERTOS_KERNEL_RP2040_RELATIVE_PATH "${FREERTOS_KERNEL_RP2040_RELATIVE_PATH}/RP2040")
            endif()
        else()
            # For rp2350-arm-s and other ARM variants, try ARM_NTZ port first
            if (EXISTS "${FREERTOS_KERNEL_PATH}/${FREERTOS_KERNEL_RP2040_RELATIVE_PATH}/RP2350_ARM_NTZ/CMakeLists.txt")
                set(FREERTOS_KERNEL_RP2040_RELATIVE_PATH "${FREERTOS_KERNEL_RP2040_RELATIVE_PATH}/RP2350_ARM_NTZ")
            else()
                message(STATUS "RP2350 ARM_NTZ port not found, using RP2040 port")
                set(FREERTOS_KERNEL_RP2040_RELATIVE_PATH "${FREERTOS_KERNEL_RP2040_RELATIVE_PATH}/RP2040")
            endif()
        endif()
    endif()
    ...
    if (NOT FREERTOS_KERNEL_PATH)
        ...
        if (_ACTUAL_PATH STREQUAL _POSSIBLE_PATH)
            ...
            break()
        elseif (PICO_SDK_PATH AND EXISTS "${PICO_SDK_PATH}/lib/FreeRTOS-Kernel")
            set(FREERTOS_KERNEL_PATH ${PICO_SDK_PATH}/lib/FreeRTOS-Kernel)
            message("Defaulting FREERTOS_KERNEL_PATH as part of PICO_SDK_PATH: ${FREERTOS_KERNEL_PATH}")
            break()
        endif()
    endif ()
